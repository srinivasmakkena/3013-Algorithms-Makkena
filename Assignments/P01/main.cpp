...
// Insert a new person into the tree based on string keys like name & address
...
// Insert a new person into the tree based on int keys like ID
...
int main()
{
  //different BST's for saving data based on different keys
  BST *bst_id = NULL;
  BST *bst_fname = NULL;
  BST *bst_lname = NULL;
  ...
  //std::ifstream file("D:\\Algos\\P01\\data.json");
  std::ifstream file("data.json");
  std::string str;

  
  //Vector for saving person data read from 
  ...
  // Inserting data to BST's and Linked List
  ...
  while (choice != 10)
  {
    ...
    if (choice!=9) p2 = search_in_LL(linkedlist, search_key);
    else if (choice == 0) p2 = search_in_LL(linkedlist, stoi(search_key));
    else p2 = search_in_LL(linkedlist, latitude, longitude);
